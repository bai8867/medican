...
- **`tcm-diet-frontend`**：Vue 3 + Vite **6**（`package.json`），开发端口 **11999**（`vite.config.js`），多键代理把 **`VITE_API_PREFIX`**（默认 `/api`）及「同源相对路径」的 **`VITE_API_BASE_URL`** 一并注册到同一 **`VITE_PROXY_TARGET`**（默认 **`http://127.0.0.1:11888`**），避免前缀与 base 不一致时静态层 **404**。`npm run dev` 前 **`predev`** 会跑 `scripts/dev-backend-probe.mjs` 并检查核心依赖（不完整则触发 `npm install`）。开发模式默认 **`VITE_AI_MOCK` 未设为 `0`** 时加载 `vite-plugins/aiApiDevMock.js`。脚本：`npm run dev` / `build` / `lint` / `typecheck` / `test` / `test:e2e`；`sync:backend-mock-json` 可将前端 mock 同步为后端 `bootstrap/mock-recipes.json`。
- **`campus-diet-backend`**：Spring Boot **2.7.18**（`pom.xml` parent），Java **11**。默认 **`server.port=11888`**，**`server.address=127.0.0.1`**（联调外机设 **`SERVER_ADDRESS=0.0.0.0`**）。**`application.yml`** 已设 **`spring.profiles.default=h2`**（未指定其它 active 时内存 H2，无需 MySQL/自设库密码）。**`config/medican-datasource.override.yml`** 仅在 **`SPRING_PROFILES_ACTIVE` 含 `mysql`** 时加载（见 **`application-mysql.yml`**）。**`启动后端.bat`** 在 **`set MEDICAN_USE_MYSQL=1`** 时会设 **`SPRING_PROFILES_ACTIVE=mysql`**。数据源默认 **`MYSQL_HOST`/`PORT`/`DATABASE`** 与 **`MYSQL_USERNAME`**（默认 **`tcm_app`**）、**`MYSQL_PASSWORD`**；也可用 **`SPRING_DATASOURCE_URL`** 覆盖整段 JDBC。SQL 初始化由 **`SPRING_SQL_INIT_MODE`**（默认非 `never` 时执行 `schema.sql`、补丁 SQL、`data.sql` 等，见 `application.yml`）控制。探活：**`GET /health`**（纯文本 `ok`）、**`GET /api/health`**（JSON `status: ok`，与联调脚本一致）、**`GET /api/health/db`**（库连通自检）。运维指标：**`management.endpoints.web.exposure.include`** 暴露 **`health`**、**`prometheus`**（即 **`/actuator/health`**、**`/actuator/prometheus`**；JWT 过滤器在 **`/api/*`**，不挡 Actuator）。大模型：**`LLM_URL`** 或 **`LLM_HTTP_HOST`/`LLM_HTTP_PORT`**、**`LLM_MODEL`**、**`LLM_API_KEY`**、**`LLM_TIMEOUT_MS`**；生产密钥与 JWT 等见 `application-prod.yml` 与基线校验类。
