...
let settingsStore: SystemSettingsState = { ...defaultSettings }
...
function safeJson(raw: unknown): Record<string, unknown> {
  ...
  if (typeof raw === 'object' && !(raw instanceof FormData) && !Array.isArray(raw)) {
...
function queryEnabled(config?: InternalAxiosRequestConfig): boolean | null {
...
export function matchAdminSystemSettingsMock(
  method: string,
  path: string,
  ...
  if (p === '/admin/system/settings' && method === 'PUT') {
    ...
    settingsStore = {
      ...settingsStore,
      site_name: (body.site_name as string | undefined) ?? settingsStore.site_name,
      contact_email: (body.contact_email as string | undefined) ?? settingsStore.contact_email,
      default_season: (body.default_season as string | undefined) ?? settingsStore.default_season,
